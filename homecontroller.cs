            ...
            var cpfExists =  _context.Users.Any(u => u.CPF == user.CPF);
                if (cpfExists)
                {
                    ModelState.AddModelError("CPF", "O CPF já está cadastrado.");
                         ...
                         var errors = ModelState.Keys.SelectMany(key => ModelState[key].Errors.Select(error => new
                {
                    FieldName = key,
                    ErrorMessage = error.ErrorMessage
                }));

                // Retorna uma resposta JSON com status de BadRequest (400)
